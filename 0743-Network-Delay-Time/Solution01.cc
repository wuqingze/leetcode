...
public:
    ...
    bool check(){
        for(int i=1;i<=len;i++){
            if(indexs[i] >0){
                if(arr[indexs[i]].v != i) return false;
            }
        }
        return check(0);;
    }
    ...
    bool check(int i){
        int l = i*2+1;
        int r = i*2+2;
        if(l>= size and r>=size) return true;
        if(l<size and arr[i].w>arr[l].w) return false;
        if(r<size and arr[i].w>arr[r].w) return false;
        bool a = check(l);
        bool b = check(r);
        return a and b;
    }
...
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        while(not heap.empty()){
            auto flag = heap.check();
            node = heap.top();
            l.push_back(node);
            mx = std::max(mx, node.w);
            cnt += 1;
            int i = node.v;
            for(int j=1;j<=n;j++){
                if(matrix[i][j] != -1){
        ...
        for(auto &item:l){
            cout<<item.v<<"\t"<<item.w<<"\t"<<item.p<<endl;
        }
        return cnt == n?mx:-1;
    }
};
